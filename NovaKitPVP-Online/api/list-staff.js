...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [authData, approved] = await Promise.all([
    ...
    const users = (authData?.users || []).filter(u => {
      ...
      const email = String(u.email || '').toLowerCase();
      return { id: u.id, email, confirmed: Boolean(u.email_confirmed_at), created_at: u.created_at || null, role: OWNER_EMAILS.has(email) ? 'owner' : 'admin' };
    });
