...
function assetField(key,label,value){return `<div class="asset-row" data-asset-row="${esc(key)}"><div class="asset-preview" data-preview="${esc(key)}">${iconMarkup(value,'asset-preview-icon')}</div><div class="grow"><label>${esc(label)}</label><input class="asset-input" data-asset="${esc(key)}" value="${esc(value)}" placeholder="Emoji, symbol, or https:// image URL"></div></div>`}
function renderAppearance(){if(!isOwner())return;const kitFields=GAMEMODES.map(g=>assetField(g.assetKey,g.name,assetValue(assets,g.assetKey))).join('');const rankFields=RANKS.map(r=>assetField(r.assetKey,r.name,assetValue(assets,r.assetKey))).join('');$('#panel').innerHTML=`<div class="panel-row"><div><h2>Appearance Icons</h2><span class="muted" style="font-size:11px">Use an emoji/symbol or paste an https:// image URL. Owner-only.</span></div><button id="saveAssets" class="btn primary small">Save Icons</button></div><div class="appearance-grid"><section class="card inner-card"><h3>Website</h3>${assetField('brand_logo','Top-left NovaKitPVP icon',assetValue(assets,'brand_logo'))}</section><section class="card inner-card"><h3>Kit Icons</h3>${kitFields}</section><section class="card inner-card"><h3>Combat Rank Icons</h3>${rankFields}</section></div>`;document.querySelectorAll('[data-asset]').forEach(i=>i.oninput=()=>{const p=document.querySelector(`[data-preview="${i.dataset.asset}"]`);if(p)p.innerHTML=iconMarkup(i.value,'asset-preview-icon')});$('#saveAssets').onclick=saveAssets}
