...
function assetField(key,label,value){const current=(value||DEFAULT_ASSETS[key]||'').trim();const usingUpload=/^data:image\//i.test(current);return `<div class="asset-row" data-asset-row="${esc(key)}"><div class="asset-preview" data-preview="${esc(key)}">${iconMarkup(current,'asset-preview-icon')}</div><div class="grow asset-input-wrap"><label>${esc(label)}</label><input class="asset-input" data-asset="${esc(key)}" value="${esc(current)}" placeholder="Paste an image URL or upload a file below"><div class="asset-upload-row"><input class="asset-file" type="file" accept=".png,.svg,.webp,.jpg,.jpeg,image/png,image/svg+xml,image/webp,image/jpeg" data-asset-upload="${esc(key)}"><button class="btn small" type="button" data-asset-default="${esc(key)}">Reset default</button></div><div class="asset-help">Upload a PNG, SVG, WEBP or JPG icon. Files are converted into a stored image value automatically, so you do not need to host them elsewhere.</div><div class="asset-flag">Current source: <b data-source-label="${esc(key)}">${usingUpload?'Uploaded image':(/^https?:\/\//i.test(current)?'Image URL':'Text / default icon')}</b></div></div></div>`}
function updateAssetPreview(key,value){const preview=document.querySelector(`[data-preview="${key}"]`);if(preview)preview.innerHTML=iconMarkup(value,'asset-preview-icon');const label=document.querySelector(`[data-source-label="${key}"]`);if(label){const v=String(value||'').trim();label.textContent=/^data:image\//i.test(v)?'Uploaded image':(/^https?:\/\//i.test(v)?'Image URL':(v===String(DEFAULT_ASSETS[key]||'').trim()?'Default icon':'Text / default icon'));}}
function wireAppearanceInputs(){document.querySelectorAll('[data-asset]').forEach(i=>i.oninput=()=>updateAssetPreview(i.dataset.asset,i.value));document.querySelectorAll('[data-asset-default]').forEach(b=>b.onclick=()=>{const key=b.dataset.assetDefault;const input=document.querySelector(`[data-asset="${key}"]`);const file=document.querySelector(`[data-asset-upload="${key}"]`);if(input)input.value=DEFAULT_ASSETS[key]||'';if(file)file.value='';updateAssetPreview(key,input?.value||'');});document.querySelectorAll('[data-asset-upload]').forEach(input=>input.onchange=async()=>{const key=input.dataset.assetUpload;const file=input.files?.[0];if(!file)return;const allowed=['image/png','image/svg+xml','image/webp','image/jpeg'];if(!allowed.includes(file.type)){toast('Please upload a PNG, SVG, WEBP or JPG file.');input.value='';return}if(file.size>600*1024){toast('Icon file is too large. Keep it under 600 KB.');input.value='';return}try{const dataUrl=await readFileAsDataURL(file);const target=document.querySelector(`[data-asset="${key}"]`);if(target){target.value=dataUrl;updateAssetPreview(key,dataUrl);}toast(`${file.name} loaded. Click Save Icons to publish it.`)}catch(e){toast(e.message||'Could not read file');}finally{input.value='';}})}
function readFileAsDataURL(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(new Error('Could not read file'));reader.onload=()=>resolve(String(reader.result||''));reader.readAsDataURL(file);})}
function renderAppearance(){if(!isOwner())return;const kitFields=GAMEMODES.map(g=>assetField(g.assetKey,g.name,assetValue(assets,g.assetKey))).join('');const rankFields=RANKS.map(r=>assetField(r.assetKey,r.name,assetValue(assets,r.assetKey))).join('');$('#panel').innerHTML=`<div class="panel-row"><div><h2>Appearance Icons</h2><span class="muted" style="font-size:11px">Owner-only uploads. PNG, SVG, WEBP and JPG are supported.</span></div><button id="saveAssets" class="btn primary small">Save Icons</button></div><div class="appearance-grid"><section class="card inner-card"><h3>Website</h3>${assetField('brand_logo','Top-left NovaKitPVP icon',assetValue(assets,'brand_logo'))}</section><section class="card inner-card"><h3>Kit Icons</h3>${kitFields}</section><section class="card inner-card"><h3>Combat Rank Icons</h3>${rankFields}</section></div>`;wireAppearanceInputs();$('#saveAssets').onclick=saveAssets}
async function saveAssets(){const rows=[...document.querySelectorAll('[data-asset]')].map(i=>({asset_key:i.dataset.asset,asset_value:i.value.trim()||DEFAULT_ASSETS[i.dataset.asset]||''}));const {error}=await supabase.from('site_assets').upsert(rows,{onConflict:'asset_key'});if(error){toast(error.message);return}await loadAssets();toast('Icons updated everywhere')}
